...
{	
	...
	{
		xmlDoc.async = false;
		xmlDoc.load("news/oneindia-news-india");
		xmlDoc2.async = false;
		xmlDoc2.load("xml/india.xml");
		...
		for(i=0; i<items.length; i++)
		{
			...
			for(k=0; k<places.length; k++)
			{
				...
				if(address == city){
				var lat_f = parseFloat(places[k].getAttribute("lat"));
				var lat_i = parseInt(places[k].getAttribute("lat"));
				var lng_f = parseFloat(places[k].getAttribute("lng"));
				var lng_i = parseInt(places[k].getAttribute("lng"));
				var lat_final = lat_i + ((lat_f - lat_i) / 0.6);
				var lng_final = lng_i + ((lng_f - lng_i) / 0.6);
				var point = new google.maps.LatLng(lat_final, lng_final);
			...
			if(k==places.length)
			{
				...
				///address2[q]=address;		/*new code for geo code*/
